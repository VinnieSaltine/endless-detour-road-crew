...
function App() {
  ...
  useEffect(() => {
    console.log("[FrontRow Firebase Project]", window.firebaseConfig?.projectId || "not configured");
  }, []);
  ...
  return (
    <section className="page admin-page">
      ...
      <div className="admin-grid">
        <AdminSection id="settings" title="Admin Settings" summary={`Application ${APP_CONFIG.version} · Firebase ${window.firebaseConfig?.projectId || "not configured"}`} expanded={adminSections.sections.settings} onToggle={() => adminSections.toggle("settings")}><div className="admin-version"><span>Application version</span><strong>{APP_CONFIG.version}</strong></div><div className="admin-version"><span>Firebase project</span><strong>{window.firebaseConfig?.projectId || "not configured"}</strong></div><p className="muted compact">Admin access is controlled by Firebase Authentication and the user's isAdmin permission.</p></AdminSection>
